  ...
  MainWindowDataContainer(this.indicators, List<Candle> candles) {
    ...
    for (var indicator in indicators) {
      for (var indicatorComponent in indicator.indicatorComponentsStyles) {
        indicatorComponentData.add(
            IndicatorComponentData(indicator, indicatorComponent.name, indicatorComponent.color));
      }
    }
    ...
    for (var indicator in indicators) {
      final List<IndicatorComponentData> containers =
          indicatorComponentData.where((element) => element.parentIndicator == indicator).toList();

      for (int i = 0; i < candles.length; i++) {
        double low = lows[i];
        double high = highs[i];

        List<double?> indicatorDatas =
            List.generate(indicator.indicatorComponentsStyles.length, (index) => null);
